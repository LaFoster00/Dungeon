...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("dgir.debugCurrentFile", async () => {
            ...
            const config = await DgirDebugConfigurationProvider.buildFromEditor(
                editor.document,
            );
            if (!config) {
                vscode.window.showErrorMessage(
                    "Unable to create DGIR debug configuration.",
                );
                return;
            }
 ...
 */
class DgirDebugConfigurationProvider
    ...
    async resolveDebugConfiguration(
        ...
        if (!config.program) {
            ...
            if (editor) {
                const inferred =
                    await DgirDebugConfigurationProvider.buildFromEditor(
                        editor.document,
                    );
                if (inferred?.program) {
        ...
        if (!config.program) {
            vscode.window.showErrorMessage(
                "DGIR: Missing 'program' (DGIR JSON path).",
            );
            return null;
        }
    ...
    async createDebugAdapterDescriptor(
        ...
        if (config.startServer !== false) {
            if (!config.jarPath || !fs.existsSync(config.jarPath)) {
                throw new Error(
                    `DGIR: jar not found at ${config.jarPath ?? "(undefined)"}. Build with ./gradlew :dgir:vm:jarDgirDebugServer`,
                );
            }
            ...
            if (useTerminal) {
                ...
                terminal.sendText(
                    [
                        shellQuote(config.javaPath ?? "java"),
                        ...args.map(shellQuote),
                    ].join(" "),
                    true,
                );
                ...
                child.on("exit", (code, signal) => {
                    const terminatedByUser =
                        signal === "SIGTERM" ||
                        code === 143 ||
                        sessionTerminating.has(session.id);
                    if (code !== 0 && !terminatedByUser) {
                        vscode.window.showErrorMessage(
                            `DGIR DAP server exited with code ${code}.`,
                        );
                    }
                });
 ...
 */
class DgirDebugAdapterTrackerFactory
...
function mapSourcePathToDgir(
    ...
    if (
        ...
        srcPath.startsWith(workspaceRoot + path.sep)
    ) {
