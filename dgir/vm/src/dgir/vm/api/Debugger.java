...
/**
 * Callback interface the {@link VM} invokes during debug-mode execution to notify a DAP adapter (or
 * any other debugger front-end) about execution events.
 *
 ...
 *   <li>{@link DebugControl#PAUSE} — block until the debugger calls {@link VM#resume()} or {@link
 *       VM#stepOver()}.
 * </ul>
 */
public interface Debugger {
   ...
   */
  @NotNull
  DebugControl onStep(@NotNull Operation operation, @NotNull Location location);
   ...
   * DebugControl#PAUSE} from a callback, or by the VM being suspended while waiting for a debugger
   * to attach).
   */
  void onPause();
}
