...
const setupStartButton = (buttons: Buttons, workspace: Blockly.WorkspaceSvg, delayInput: HTMLInputElement) => {
  buttons.startBtn.addEventListener("click", async () => {
    ...
    const error = validateProgram(code);
...
const validateProgram = (fullProgram: string) => {
  ...
  const rules = [
    ...
    {
      check: () => isMissingDirectionInIsNearTile(fullProgram, "WALL"),
      msg: "Das Wand-Element braucht eine Richtungangabe"
    },
    {
      check: () => isMissingDirectionInIsNearTile(fullProgram, "FLOOR"),
      msg: "Das Boden-Element braucht eine Richtungsangabe"
    },
    {
      check: () => isMissingDirectionInIsNearTile(fullProgram, "PIT"),
      msg: "Das Loch-Element braucht eine Richtungsangabe."
    },
    {
      check: () => isMissingDirectionInIsNearComponent(fullProgram, "LeverComponent"),
      msg: "Schalter/Fackel braucht eine Richtungsangabe."
    },
    {
      check: () => isMissingDirectionInIsNearComponent(fullProgram, "AIComponent"),
      msg: "Das Monster-Element braucht eine Richtungsangabe"
    },
    ...
    {
      check: () => hasIncompleteIfComparison(fullProgram),
      msg: "Bei einem Vergleich muss eine Wert auf beiden Seiten stehen"
    },
    {
      check: () => containsString(fullProgram, "no direction in rotation"),
      msg: "Eine Rotation muss eine Richtungsangabe enthalten"
    },
    {
      check: () => containsString(fullProgram, "no value was assigned to the variable"),
      msg: "Der Variable wurde keine Wert zugewiesen"
    },
